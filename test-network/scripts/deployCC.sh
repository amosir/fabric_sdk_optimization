...
packageChaincode() {
    ...
    set +x
    cat log.txt
    verifyResult $res "Chaincode packaging on peer$i.org${ORG} has failed"
    echo "===================== Chaincode is packaged on peer$i.org${ORG} ===================== "
    echo
  done
}
...
approveForMyOrg() {
	...
	setGlobals $ORG 0
    set -x
    peer lifecycle chaincode approveformyorg -o localhost:17050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
    res=$?
    set +x
    cat log.txt
...
commitChaincodeDefinition() {
	...
	set -x
	peer lifecycle chaincode commit -o localhost:17050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
	res=$?
	set +x
	cat log.txt
...
chaincodeInvokeInit() {
	...
	peer chaincode invoke -o localhost:17050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
	res=$?
	set +x
	cat log.txt
